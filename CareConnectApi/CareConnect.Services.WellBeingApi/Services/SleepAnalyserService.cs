...
namespace CareConnect.Services.WellBeingApi.Services
{
    public class SleepAnalyserService : ISleepAnalyserService
    {
        ...
        public IEnumerable<SleepAnalyser> GetUserSleepLog(int userid)
        {
        ...
        public SleepAnalyser GetDataByUserDate(int userid, DateOnly date)// get by id and date . 
        {
            try
            {
        ...
        public bool DeleteUserSleepLog(int userid,DateOnly date)
        {
            if (date != DateOnly.MinValue)
            {
                var result = _db.SleepAnalyser.FirstOrDefault(u => u.DateTimeOfEntry == date && u.UserId==userid);
                if (result != null)
                {
