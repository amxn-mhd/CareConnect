...
namespace CareConnect.Services.SafetyApi.Services
{
        ...
        public class ReportIncidentDtoService : IReportIncidentDtoService
        {
         ...
         public async Task<IEnumerable<ReportIncidentDto>> GetUserEventByDate(string id, DateOnly date)
        {
            try
            {
                var result = await _db.ReportIncident.Where(i => i.DateTimeOfEntry == date && i.UserId .Equals(id)).ToListAsync();
            ...
            public  async Task<IEnumerable<ReportIncidentDto>>GetUserEventData(string id)
            {
            try
            {
                var result = await _db.ReportIncident.Where(i => i.UserId.Equals(id)).ToListAsync();
                return _mapper.Map<IEnumerable<ReportIncidentDto>>(result);
            }
            catch(Exception ex)
            {
